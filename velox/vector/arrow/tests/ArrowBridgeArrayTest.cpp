...
class ArrowBridgeArrayExportTest : public testing::Test {
 protected:
  ...
  void testFlatVector(
    ...
    velox::exportToArrow(flatVector, options_, arrowArray, pool_.get());
  ...
  void testArrayVector(const T& inputData) {
    ...
    ArrowArray arrowArray;
    velox::exportToArrow(arrayVector, options_, arrowArray, pool_.get());
  ...
  void testConstantVector(
      ...
      const TInput& input) {
    ArrowArray arrowArray;
    velox::exportToArrow(constantVector, options_, arrowArray, pool_.get());
  ...
  void exportToArrow(const TypePtr& type, ArrowSchema& out) {
    velox::exportToArrow(
        BaseVector::create(type, 0, pool_.get()), options_, out);
  }
...
TEST_F(ArrowBridgeArrayExportTest, flatTimestamp) {
  for (uint8_t unit = 0; unit <= static_cast<uint8_t>(TimestampUnit::kNano);
       ++unit) {
...
TEST_F(ArrowBridgeArrayExportTest, rowVector) {
  ...
  ArrowArray arrowArray;
  velox::exportToArrow(vector, options_, arrowArray, pool_.get());
...
TEST_F(ArrowBridgeArrayExportTest, rowVectorEmpty) {
  ArrowArray arrowArray;
  velox::exportToArrow(
      vectorMaker_.rowVector({}), options_, arrowArray, pool_.get());
  EXPECT_EQ(0, arrowArray.n_children);
  EXPECT_EQ(1, arrowArray.n_buffers);
  EXPECT_EQ(nullptr, arrowArray.children);
...
std::shared_ptr<arrow::Array> toArrow(
  ...
  ArrowSchema schema;
  ArrowArray array;
  exportToArrow(vec, options, schema);
  exportToArrow(vec, options, array, pool);
...
TEST_F(ArrowBridgeArrayExportTest, arrayDictionary) {
  ...
  ArrowSchema schema;
  ArrowArray data;
  velox::exportToArrow(vec, options_, schema);
  velox::exportToArrow(vec, options_, data, vec->pool());
...
class ArrowBridgeArrayImportTest : public ArrowBridgeArrayExportTest {
 ...
 private:
  ...
  void testArrowRoundTrip(const arrow::Array& array, F validateVector) {
    ...
    velox::exportToArrow(vec, options_, schema);
    velox::exportToArrow(vec, options_, data, pool_.get());
