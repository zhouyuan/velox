...
HashBuild::HashBuild(
  ...
  if (reusedHashTableAddress_ != nullptr) {
    ...
    if (!dropDuplicates_) {
      if (numDependents > 0) {
        // Number of join keys (numKeys) may be less then number of input
        // columns (inputType->size()). In this case numDependents is negative
        // and cannot be used to call 'reserve'. This happens when we join
        // different probe side keys with the same build side key: SELECT * FROM
        // t LEFT JOIN u ON t.k1 = u.k AND t.k2 = u.k.
